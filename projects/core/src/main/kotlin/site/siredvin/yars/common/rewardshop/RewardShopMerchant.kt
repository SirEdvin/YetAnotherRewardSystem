...
class RewardShopMerchant(
    ...
    override fun notifyTrade(offer: MerchantOffer) {
        ...
        val trade = RewardShopTrades.all().firstOrNull { it.id == tradeId } ?: return
        val resolved = trade.resolve(RewardShopTradeHistory.completed(player, tradeId)) ?: return
    ...
    private fun rebuildOffers() {
        ...
        RewardShopTrades.all().forEach { trade ->
            val resolved = trade.resolve(RewardShopTradeHistory.completed(player, trade.id)) ?: return@forEach
